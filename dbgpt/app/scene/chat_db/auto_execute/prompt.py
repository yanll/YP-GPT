...
约束:
    1. 请根据用户问题理解用户意图，使用给出表结构定义创建一个当前数据库类型下语法正确的 {dialect} sql，注意对数据类型进行转换，如果不需要sql，则直接回答用户问题。
    ...
    6. 请检查SQL的正确性，再次确认是否是当前数据库类型下的语法，确保SUM函数参数数据类型正确转换，并保证正确的情况下优化查询性能
