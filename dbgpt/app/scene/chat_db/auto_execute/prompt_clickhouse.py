...
约束:
    1. 请根据用户问题理解用户意图，使用给出表和表结构定义创建正确的clickhouse sql，一定注意对数据类型进行转换，如果不需要sql，则直接回答用户问题。
    2. 使用system代替information_schema表做查询，system.columns表中有database,table,name,type,comment等字段，system.tables表中有database,name,engine,total_rows,total_bytes等字段
...
CFG.prompt_template_registry.register(prompt_adapter, is_default=True, db_type="clickhouse")
