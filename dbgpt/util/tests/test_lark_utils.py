...
def test_select_buildings():
    token = larkutil.get_tenant_access_token()['tenant_access_token']
    lark_calendar_util.select_buildings(token)
...
def test_select_rooms():
    token = larkutil.get_tenant_access_token()['tenant_access_token']
    lark_calendar_util.select_rooms(token, "omb_2cbd7aac59a2292a577547e6d5ef6300")
...
def test_create_calendar_id():
    token = larkutil.get_tenant_access_token()['tenant_access_token']
    lark_calendar_util.create_calendar_id(token=token)
    assert True
...
def test_grant_calendar():
    ...
    lark_calendar_util.grant_calendar(token, calendar_id, "writer", "ou_1a32c82be0a5c6ee7bc8debd75c65e34")
    assert True
