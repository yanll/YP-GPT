...
def monitor4():
    ...
    for PAYER_CUSTOMER_SIGNEDNAME in CUSTOMER1['PAYER_CUSTOMER_SIGNEDNAME_list']:
        if PAYER_CUSTOMER_SIGNEDNAME not in CUSTOMER1['PAYER_CUSTOMER_SIGNEDNAME_to_SUCCESS_AMOUNT_this_week']:
            CUSTOMER1['PAYER_CUSTOMER_SIGNEDNAME_to_SUCCESS_AMOUNT_this_week'][PAYER_CUSTOMER_SIGNEDNAME] = 0
        if PAYER_CUSTOMER_SIGNEDNAME not in CUSTOMER1['PAYER_CUSTOMER_SIGNEDNAME_to_SUCCESS_AMOUNT_this_week']:
            CUSTOMER1['PAYER_CUSTOMER_SIGNEDNAME_to_SUCCESS_AMOUNT_this_week'][PAYER_CUSTOMER_SIGNEDNAME] = 0
    ...
    alert_list = []
    for PAYER_CUSTOMER_SIGNEDNAME in CUSTOMER1['PAYER_CUSTOMER_SIGNEDNAME_list']:
        PAYER_CUSTOMER_SIGNEDNAME_to_SUCCESS_AMOUNT_this_week = CUSTOMER1['PAYER_CUSTOMER_SIGNEDNAME_to_SUCCESS_AMOUNT_this_week'][PAYER_CUSTOMER_SIGNEDNAME]


        content = None
        if PAYER_CUSTOMER_SIGNEDNAME_to_SUCCESS_AMOUNT_this_week >= 1500 * (10 ** 4):
            fluctuation = (CUSTOMER1['PAYER_CUSTOMER_SIGNEDNAME_to_SUCCESS_AMOUNT_this_week'][
                               PAYER_CUSTOMER_SIGNEDNAME] /
                           CUSTOMER1['PAYER_CUSTOMER_SIGNEDNAME_to_SUCCESS_AMOUNT_last_week'][
                               PAYER_CUSTOMER_SIGNEDNAME]) - (
                                  CUSTOMER1['SUCCESS_AMOUNT_this_week'] / CUSTOMER1['SUCCESS_AMOUNT_last_week'])
            if fluctuation <= -0.15:
                content = f'付方名称:{PAYER_CUSTOMER_SIGNEDNAME}，航司:ZH——商编:10012407595+场景字段，近7天充值金额，环比上周下降{fluctuation*100:.2f}%，高于/低于大盘**'
            if fluctuation >= 0.20:
                content = f'付方名称:{PAYER_CUSTOMER_SIGNEDNAME}，航司:ZH——商编:10012407595+场景字段，近7天充值金额，环比上周上升{fluctuation*100:.2f}%，高于/低于大盘**'
        ...
        if content is not None:
            alert_list.append({'name': sale_name1, 'title': '深航/国航充值业务',
                              'content': content})
