...
class Monitor1ByPayerDataProvider(AirlineMonitorDataProvider):
    ...
    def __get_data_by_payer_in_monitor1(self, trx_date: str, payer_sales_name=None, payer_customer_signedname=None):
        parameters = {
            "TRX_DATE": trx_date
        }
        try:
            resp = self.dmall_client.post(
                api_name="new_get_data_by_payer_in_montor1",
    ...
    def new_get_sales_and_payers_by_stat_in_montor1(self, trx_date: str):
        parameters = {
            "TRX_DATE": trx_date
        }
        ...
        try:
            resp = self.dmall_client.post(
                api_name="new_get_sales_and_payers_by_stat_in_montor1",
                parameters=parameters
            )
            resp = resp.json()
            data = resp['data']['data']
    ...
    def new_get_reason_4_data_by_payer_in_monitor1(self, trx_date: str):
        try:
            resp = self.dmall_client.post(
                api_name="new_get_reason_4_data_by_payer_in_montor1",
                parameters={
                    'TRX_DATE': trx_date,
                }
            )
    ...
    def new_get_reason_5_data_by_payer_in_monitor1(self, trx_date: str):
        try:
            resp = self.dmall_client.post(
                api_name="new_get_reason_5_data_by_payer_in_montor1",
                parameters={
                    'TRX_DATE': trx_date
                }
            )
